...
def model_for(role: str) -> str:
    specific = os.getenv("MISMAR_MODEL_" + role.upper())
    return specific or os.getenv("MISMAR_MODEL") or "openai/gpt-4o-mini"
...
def debate(request: str, mode: str = "auto", rounds: int = 2) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=min(len(roles), 6)) as pool:
    ...
    # Debate uses compressed opinions, not full histories.
        ...
        with ThreadPoolExecutor(max_workers=min(len(roles), 6)) as pool:
            futures = {
                pool.submit(
                    ...
                    r,
                    request,
                    "Critique the following council digest. Identify only the most important disagreement or blind spot.\n" + digest,
                    260,
                ): r
                for r in roles
            }
